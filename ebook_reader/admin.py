...
class IndexItemInline(admin.TabularInline):
    model = IndexItem
    fields = ("title", "start_page", "end_page")
    extra = 1
...
class BookAdmin(admin.ModelAdmin):
    ...
    actions = ("reextract_selected_books",)

    def get_queryset(self, request):
        return super().get_queryset(request).annotate(_page_count=Count("pages"))

    @admin.display(ordering="_page_count", description="Pages")
    def page_count(self, obj):
        return obj._page_count

    @admin.display(description="Progress")
    def processing_progress(self, obj):
        return f"{obj.processed_pages}/{obj.total_pages or '?'}"

    @admin.display(description="Extracted pages")
    def pages_admin_link(self, obj):
        if not obj.pk:
            return "Save the book first."
        url = reverse("admin:ebook_reader_bookpage_changelist")
        return format_html(
            '<a href="{}?book__id__exact={}">Review/edit pages</a>',
            url,
            obj.pk,
        )

    @admin.action(description="Re-extract selected PDFs (OCR when needed)")
    def reextract_selected_books(self, request, queryset):
        if getattr(settings, "EBOOK_READER_ASYNC_PROCESSING", False):
            from .tasks import process_uploaded_book

            for book in queryset:
                process_uploaded_book.delay(book.pk)
            self.message_user(
                request,
                f"Queued {queryset.count()} book(s) for extraction.",
                messages.SUCCESS,
            )
            return
        completed = 0
        ...
        for book in queryset:
            try:
                ...
                failures.append(f"{book}: {error}")
                logger.exception("Manual PDF re-extraction failed for book pk=%s", book.pk)
        if completed:
            self.message_user(request, f"Re-extracted {completed} book(s).", messages.SUCCESS)
        if failures:
            self.message_user(
                request,
                f"{len(failures)} book(s) failed. First error: {failures[0]}",
                messages.ERROR,
            )
...
class HighlightAdmin(admin.ModelAdmin):
    list_display = ("user", "page", "color", "created_at")
    list_filter = ("color", "created_at")
    search_fields = ("user__username", "page__book__title", "selected_text")
...
class BookPageAdmin(admin.ModelAdmin):
    list_display = ("book", "page_number", "line_count", "extraction_method")
    list_filter = ("book", "extraction_method")
    search_fields = ("book__title", "plain_text", "content")
    autocomplete_fields = ("book",)


@admin.register(IndexItem)
class IndexItemAdmin(admin.ModelAdmin):
    list_display = ("title", "book", "start_page", "end_page")
    list_filter = ("book",)
