...
def _candidate_from_line(row: TocRow, strategy_name: str) -> TocCandidate | None:
    ...
    if not text:
        return None
    noisy_parts = _candidate_parts_from_noisy_table_line(text)
    if noisy_parts:
        order, title, page_number = noisy_parts
    else:
    ...
    if not _is_meaningful_title(title) or _is_devanagari_footer_or_note(title):
        return None
    confidence, reasons = score_candidate(
        row=row,
        order=order,
...
def _candidate_parts_from_noisy_table_line(text: str) -> tuple[int | None, str, int | None] | None:
    value = normalize_text(text)
    number_matches = list(re.finditer(r"[0-9०-९]+", value))
    if len(number_matches) < 2:
        return None
    order = parse_confident_integer(number_matches[0].group(0), structural_hint=True)
    page_number = parse_confident_integer(number_matches[-1].group(0), structural_hint=True)
    if order is None or page_number is None:
        return None
    title_text = value[number_matches[0].end():number_matches[-1].start()]
    title = _clean_title(_strip_table_noise(title_text))
    if not title:
        return None
    return order, title, page_number
