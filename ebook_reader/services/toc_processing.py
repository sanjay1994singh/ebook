...
def _extract_toc_page_with_ocr(ebook_document, page_number, *, embedded_text, ocr_engine):
    ...
    if _ocr_text_has_table_lines(result.full_text):
        page_warnings.append(
            f"PDF page {page_number} OCR line text looked structured; line parser was preferred."
        )
        return TocPageInput(
            page_number=page_number,
            embedded_text=result.full_text,
            ocr_words=[],
...
def _ocr_text_has_table_lines(text):
    lines = [line.strip() for line in (text or "").splitlines() if line.strip()]
    if len(lines) < 2:
        return False
    structured = 0
    for line in lines:
        digit_count = sum(char.isdigit() for char in line) + sum("\u0966" <= char <= "\u096f" for char in line)
        devanagari_count = sum("\u0900" <= char <= "\u097f" for char in line)
        if digit_count >= 2 and devanagari_count >= 3:
            structured += 1
    return structured >= 2
