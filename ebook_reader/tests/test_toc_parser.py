...
class TocParserTests(TestCase):
    ...
    def test_noisy_tesseract_table_lines_parse_as_separate_lessons(self):
        page = TocPageInput(
            page_number=13,
            embedded_text=(
                "4| १०७. | झूलत डोल दोऊ जन ठाड़े १३३.\n"
                "£| १०८. | झूलत डोल श्री कुँजबिहारी १३४. है\n"
                "$#| १०६. | कौन प्रकृति तिहारी छिया १३५. है\n"
                "«| ११०. | जुव कवनी वैस किसोर दोऊ १३६ Ie\n"
                "$ श्रीस्वामी राधासरण देवजू महाराज जी द्वारा\n"
                ": केलिमाल जी की :\n"
                "“ वस्तु दर्शनी भाव वर्णन टीका हर\n"
                "* (१३८ से २१८ तक )"
            ),
        )

        result = parse_toc(parser_input([page], start=13, end=13))

        self.assertEqual([item.order for item in result.valid_candidates], [107, 108, 106, 110])
        self.assertEqual(result.valid_candidates[0].title, "झूलत डोल दोऊ जन ठाड़े")
        self.assertEqual(result.valid_candidates[0].printed_page_number, 133)
        self.assertFalse(any("द्वारा" in item.title for item in result.valid_candidates))
