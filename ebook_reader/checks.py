...
def _ocr_checks():
    ...
    configured_executable = str(getattr(settings, "EBOOK_TESSERACT_CMD", "")).strip()
    executable = configured_executable or shutil.which("tesseract")
    ...
    if "hin" in languages:
        try:
            ...
            tessdata_dir = str(getattr(settings, "EBOOK_TESSDATA_DIR", "")).strip()
            if tessdata_dir:
                process_env["TESSDATA_PREFIX"] = tessdata_dir
