...
class Book(models.Model):
    ...
    class Meta:
        ordering = ("title", "id")
        indexes = [
            models.Index(fields=("processing_status",), name="ebook_book_status_idx"),
            models.Index(fields=("uploaded_at",), name="ebook_book_uploaded_idx"),
        ]
...
class BookPage(models.Model):
    class ExtractionMethod(models.TextChoices):
        EMBEDDED = "embedded", "Embedded Unicode text"
        LEGACY_CONVERTER = "legacy_converter", "Legacy font converter"
        OCR = "ocr", "Tesseract OCR"
        FALLBACK = "fallback", "Unconverted fallback"
    ...
    extraction_method = models.CharField(
        ...
        default=ExtractionMethod.EMBEDDED,
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(
                fields=("book", "extraction_method"),
                name="ebook_page_book_method_idx",
            )
        ]
        constraints = [
            models.UniqueConstraint(
                fields=("book", "page_number"),
                name="ebook_reader_unique_book_page",
            )
        ]

    def __str__(self):
        return f"{self.book} - page {self.page_number}"


class IndexItem(models.Model):
    book = models.ForeignKey(Book, related_name="index_items", on_delete=models.CASCADE)
    title = models.CharField(max_length=255)
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                check=Q(end_page__gte=F("start_page")),
                name="ebook_reader_index_end_gte_start",
            )
        ]
    ...
    def clean(self):
        ...
        if (
            ...
            raise ValidationError({"end_page": "End page must be on or after start page."})
        if self.book_id and self.book.total_pages:
            errors = {}
            if self.start_page is not None and self.start_page > self.book.total_pages:
                errors["start_page"] = "Start page exceeds the book's total pages."
            if self.end_page is not None and self.end_page > self.book.total_pages:
                errors["end_page"] = "End page exceeds the book's total pages."
            if errors:
...
class Highlight(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=("user", "page"), name="ebook_highlight_user_page_idx")
        ]
